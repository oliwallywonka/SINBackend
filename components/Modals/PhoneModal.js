...
import {usePutPhone,usePostPhone} from '../../hooks/usePhone'

export default function Modal() {

  const validate = Yup.object({
    phone: Yup.string()
      .min(8,'El numero introducido es muy corta')
      .required('El numero es obligatoria')
  });

  const {hideModal,visible,payload} = useContext(ModalContext);
  const [handlePostRequest] = usePostPhone();
  const [handlePutRequest] = usePutPhone();
              ...
              <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                ...
                <div className="flex items-start justify-between p-5 border-b border-solid border-blueGray-200 rounded-t">
                  <h4 className="text-2xl font-semibold">
                    {payload=== null ? 'Nuevo':'Editar'} Numero
                  </h4>
                  <button
                ...
                <Formik
                  enableReinitialize
                  initialValues={{
                    phone: payload===null?'':payload.phone,
                  }}
